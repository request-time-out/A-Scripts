...
// Type: AIProject.SetDesiredAllNearAction
    ...
    public virtual void OnStart()
    {
      ...
      switch (Singleton<Manager.Map>.Instance.Simulator.Weather)
      {
        ...
        case Weather.Storm:
          ...
          {
            SetDesiredAllNearAction.CreateList(agent, chunk.AppendActionPoints, actionPointList, this._eventType, this._checkFollowType, true);
            if (!actionPointList.IsNullOrEmpty<ActionPoint>())
              SetDesiredAllNearAction.NearestPoint(position, actionPointList, out this._destination);
            if (Object.op_Equality((Object) this._destination, (Object) null))
            {
              actionPointList.Clear();
              SetDesiredAllNearAction.CreateList(agent, chunk.ActionPoints, actionPointList, this._eventType, this._checkFollowType, true);
              if (!actionPointList.IsNullOrEmpty<ActionPoint>())
              {
                SetDesiredAllNearAction.NearestPoint(position, actionPointList, out this._destination);
                break;
      ...
      {
        actionPointList.Clear();
        SetDesiredAllNearAction.CreateList(agent, chunk.AppendActionPoints, actionPointList, this._eventType, this._checkFollowType, false);
        if (!actionPointList.IsNullOrEmpty<ActionPoint>())
          SetDesiredAllNearAction.NearestPoint(position, actionPointList, out this._destination);
      }
      if (Object.op_Equality((Object) this._destination, (Object) null))
      {
        actionPointList.Clear();
        SetDesiredAllNearAction.CreateList(agent, chunk.ActionPoints, actionPointList, this._eventType, this._checkFollowType, false);
        if (!actionPointList.IsNullOrEmpty<ActionPoint>())
          SetDesiredAllNearAction.NearestPoint(position, actionPointList, out this._destination);
      }
        ...
        if (!Object.op_Equality((Object) actionPoint1, (Object) null) && !Object.op_Equality((Object) actionPoint1.OwnerArea, (Object) null) && (actionPoint1.IsNeutralCommand && !actionPoint1.IsReserved(agent)))
        {
          ...
          if (!isRain || actionPoint1.AreaType == MapArea.AreaType.Indoor)
          {
            ...
            if (flag1 && (!isFollow ? actionPoint1.AgentEventType : actionPoint1.AgentDateEventType).Contains(eventType))
            {
              ...
              if (agent.NavMeshAgent.CalculatePath(actionPoint1.LocatedPosition, SetDesiredAllNearAction._navMeshPath) && SetDesiredAllNearAction._navMeshPath.get_status() == null && NavMesh.SamplePosition(actionPoint1.LocatedPosition, ref navMeshHit, meshSampleDistance, agent.NavMeshAgent.get_areaMask()))
