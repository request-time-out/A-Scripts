...
// Type: AIProject.CaptionScript.TextCommand
...
namespace AIProject.CaptionScript
{
  public class TextCommand : IScriptCommand
  {
    ...
    public bool Execute(Dictionary<string, string> table, int line)
    {
        ...
        if (int.TryParse(s, out result))
        {
          switch (result)
          {
            case 0:
              break;
            case 1:
              Singleton<ADV>.Instance.Captions.CaptionSystem.SetName(((Object) ((Component) Singleton<ADV>.Instance.TargetCharacter.ChaControl).get_gameObject()).get_name());
              break;
            default:
      ...
      Singleton<ADV>.Instance.Captions.CaptionSystem.Action = (Action) (() => this.OnComplete());
